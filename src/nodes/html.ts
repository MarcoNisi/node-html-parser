...
class DOMTokenList {
  ...
  public constructor(
    valuesInit: string[] = [],
    afterUpdate: (t: DOMTokenList) => void = () => null
  ) {
...
export default class HTMLElement extends Node {
  ...
  public get structure() {
    ...
    function dfs(node: HTMLElement) {
      ...
      const classStr = node.classList.length
 ...
 */
export function base_parse(
  data: string,
  options = { lowerCaseTagName: false, comment: false } as Partial<Options>
) {
  ...
  const kIgnoreElements = element_names
    .filter((it) => {
      return elements[it];
    })
    .map((it) => {
  ...
  while ((match = kMarkupPattern.exec(data))) {
    ...
    if (!match[1]) {
      ...
      const tagName = currentParent.rawTagName as
        | "LI"
        | "P"
        | "B"
        | "TD"
          ...
          if (text.length > 0 && /\S/.test(text)) {
            currentParent.appendChild(
              new TextNode(
                text,
                currentParent,
                createRange(tagEndPos, textEndPos)
              )
            );
          }
        }
    ...
    if (match[1] || match[4] || kSelfClosingElements[match[2]]) {
      while (true) {
        if (currentParent.rawTagName === match[2]) {
          ...
          (<[number, number]>currentParent.range)[1] = createRange(
            -1,
          ...
          const tagName = currentParent.tagName as
            | "LI"
            | "A"
            | "B"
            | "I"
